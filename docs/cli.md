...
controller. It is the third consumer of [`@avatarsd-llc/strawberry-cli`](./library.md) (the
...
- Runtime: Node 22+ (the built-in global `WebSocket` is used; on older Node the optional `ws`
  package is needed).
- Protocol: see [`protocol.md`](./protocol.md). The CLI's vocabulary is generated from the
...
(see [`protocol.md`](./protocol.md) quirk 1), so a new process replaying the stored token gets
