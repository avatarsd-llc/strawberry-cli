...
| reach-and-auth | [`reach-and-auth/`](./reach-and-auth/) | Discover a board on the LAN (WS probe, no mDNS), confirm by MAC, HMAC login, persist token to a 0600 store for resume across reboots. The mandatory first step. | `discover`, `auth login`, `auth resume`, `query capabilities`, `query wifi` |
...
| config-hardware | [`config-hardware/`](./config-hardware/) | Persisted hardware/runtime config + boot-time subsystem flags (1-wire/modbus/zigbee/can, pending-reboot) + 1-Wire IO boards. | `system config`, `system flags`, `ow-config apply`, `ow-config get`, `query system_flags` |
