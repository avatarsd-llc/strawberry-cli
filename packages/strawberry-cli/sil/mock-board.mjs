...
/* The codec comes from the BUILT library (@avatarsd-llc/strawberry-client, the
   workspace-linked sibling package) so the mock cannot drift from the schema the
   CLI actually encodes/decodes. `npm run build -w @avatarsd-llc/strawberry-client`
   must have run so its dist/ exists. */
const {
  ClientMessage, ServerMessage, Query_What, ErrCode, GrowKind,
} = await import('@avatarsd-llc/strawberry-client');
